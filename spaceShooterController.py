...
windowSurface = pygame.display.set_mode((WINDOWWIDTH, WINDOWHEIGHT), (FULLSCREEN|HWSURFACE|DOUBLEBUF), 32)
...
while True:
	...
	while True: # the game loop runs while the game part is playing
		...
		if controller.get_axis(0) > 0.15 or controller.get_axis(0) < -0.15:
			accelX = round(controller.get_axis(0), 1) - 0.1 * SENSITIVITY
		if controller.get_axis(1) > 0.15 or controller.get_axis(1) < -0.15:
			accelY = round(controller.get_axis(1), 1) - 0.1 * SENSITIVITY
		if controller.get_axis(0) < 0.15 and controller.get_axis(0) > -0.15:
			accelX = 0
		if controller.get_axis(1) < 0.15 and controller.get_axis(1) > -0.15:
			accelY = 0
